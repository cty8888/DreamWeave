...
| 日期 | 问题 | 原因 | 解决方案 | 关键词 |
|------|------|------|----------|--------|
| 2026-06-13 | 按场景筛选永远落空、卡片显示数字 | `scene_ids` 对预设场景存的是数字 id，而筛选/显示按名字 | 统一改存场景名 + 启动幂等迁移把旧数据 id→名字 | scene_ids, data-consistency |
| 2026-06-13 | 社交接口可越权操作私有梦境 | 点赞/评论未校验 `visibility` | 加 `loadPublicDream`，非公开返回 403 | authz, IDOR |
| 2026-06-13 | AI 功能其实跑不起来但测试全绿 | `llm.js` 是抛异常的桩，测试全 mock | 补全真实 OpenAI 调用 + 无 Key 离线降级 | llm, stub, mock |

---

## 迭代扩展阶段（初版 30 任务之后）

### 2026-06-13 — 功能扩展 + 安全加固 + UI 重构

- **触发的技能**：`frontend-design`、`code-review`、`security-review`
- **内容**：
  - 新功能：AI 解梦、续写风格、搜索筛选、点赞评论、关注/主页/通知、数据图谱、编辑/删除/草稿（详见 PLAN T31–T42、SPEC §11）
  - 账号体系：用户名+密码，移除 email
  - LLM：桩 → 真实调用 + 离线降级
  - 安全：限流 / CORS 白名单 / 生产 JWT / Prompt 注入防护 / 社交可见性鉴权 / 评论限长
  - UI：Oneiric Nocturne 设计系统 + 成体系动效
- **code-review 发现并修复**：`scene_ids` 存数字 id 导致按场景筛选失效 + 卡片显示数字 → 统一存名字 + 幂等迁移
- **security-review 发现并修复**：社交接口越权、登录无限流、JWT 默认密钥、CORS 全开、Prompt 注入
- **测试**：27 → 45（新增 social / stats / 关注通知 / 迁移 / 解梦等用例）
- **教训 & 可复用模板**：
  - **测试数据要贴近真实写入路径**：scene_ids 的测试用了名字字符串，恰好"骗过"了测试，掩盖了生产里实际存数字 id 的 bug。
  - **mock 让测试变绿 ≠ 功能可用**：LLM 桩 + 全 mock 的测试通过，但真实 AI 功能其实抛异常。
  - **加功能时最易漏鉴权**：新增社交接口顺手漏了可见性校验，是新增代码典型的安全债——security-review 专门盯这类。
  - **设计系统是高杠杆改动**：一层全局 CSS + 共享组件，改一处全站受益，验证了"先功能后打磨"在 agentic 流程下可行。

### 2026-06-14 — 随机/朗读、部署修复、生态填充与推送

- **触发的技能**：n/a（手动迭代）
- **内容**：
  - 新增随机漫游 `GET /dreams/random`、浏览器 TTS 朗读；修复前端 API baseURL（硬编码 localhost → 相对路径，解决远程部署打不开）
  - **生态种子脚本** `seed-ecosystem.js`：直接写库（不调 LLM、AI 文本预写），清库后生成 30 账号 + 64 梦境 + 65 接力 + 97 评论 + 416 点赞 + 155 关注 + 94 收藏 + 68 碎片 + 561 通知，时间戳散布近 30 天，构成"非冷启动"社区
  - 账号密码写入 `ACCOUNTS.md`（gitignore，不入库）
  - 提交分层为 server / client / docs / chore 四类，推送到 GitHub（origin/master）
- **教训 & 可复用模板**：
  - **示例数据用脚本生成、不入库**：仓库只存可复现的脚本，clone 后一条命令重建生态，既不泄露密码也保证一致性
  - **直接写库优于走 API 填充**：批量造数据时绕开 LLM 与限流，确定性随机 + 事务插入，几百行数据秒级完成
  - 凭据类文件务必先进 `.gitignore` 再生成，避免误提交
